...
var sheet = ss.getSheetByName('ArduinoIOTCloudSensorData');


var MAX_ROWS = 1344;     // max number of data rows to display
// 14 days * 4 (per hour) * 24 hours
var HEADER_ROW = 1;     // row index of header
var TIMESTAMP_COL = 1;  // column index of the timestamp column
var INTERVAL_IN_SECONDS = 15*60;  // 15 minutes
...
function doPost(e) {  
  ...
  if (lastRow > 1) { // this is not the first time - check if we have a new date
    ...
    if (date.getTime() - lastDate.getTime() > (INTERVAL_IN_SECONDS*1000 / 2)) { 
